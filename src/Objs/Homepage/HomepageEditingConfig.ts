import { load, provideEditingConfig } from 'scrivito'
...
load(() => jwtPisaSalesConfigSite()).then((jwtSite) => {
  provideEditingConfig(Homepage, {
    ...
    attributes: {
      ...
      siteTwitterSite: {
        title: 'Twitter site',
        description:
          'Needs to be approved at https://cards-dev.x.com/validator',
      },
    ...
    propertiesGroups: (site) => [
      ...defaultPagePropertiesGroups,
      {
        ...
        properties: [
          'contentTitle',
          'baseUrl',
          site.id() === jwtSite?.id() ? 'jwtPisaSalesApiUrl' : null,
          'siteLogoDark',
          'siteFavicon',
          'siteLanguageIcon',
          'siteCartPage',
